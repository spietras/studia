...
{
    return vec3(light.ambientIntensity) * light.color * texture(material.diffuse, TexCoords).rgb;
}
...
vec3 getDiffuse(Material material, PointLight light, vec3 norm, vec3 lightDir)
{
    ...
    return vec3(light.diffuseIntensity) * light.color * diff * texture(material.diffuse, TexCoords).rgb;
}
