...
void Game::draw()
{
    ...
    for(const auto& door : currentRoom_.getDoors())
	{
		if(door.getBody().getTextureRect().intersects(viewInt))
		{
	...
	for(const auto& key : currentRoom_.getKeys())
	{
		if(key.getBody().getTextureRect().intersects(viewInt))
		{
...
Game::Game(sf::VideoMode mode, std::string title) : window_(mode, title)
{
	...
	currentRoom_ = Room(Resources::getRoomId(Resources::map_.at("startingRoom").get<std::string>()), openedDoors_);
