...
```{r}
feature_selection_methods = c("correlation", "variable_importance")
feature_selection_score = 1

helper_task <- build_class_task(data, "minmax")
features <- hashmap()
features[['variable_importance']] <- select_features(helper_task, "variable_importance", feature_selection_score)
features[['correlation']] <- select_features(helper_task, "correlation", feature_selection_score)
```
...
for (feature_selection_method in feature_selection_methods) {
  for (sampling in sampling_methods) {
    for (scaling in scaling_methods) {
      new_task <- build_class_task(data, scaling, id=sprintf("%s_%s_%s", scaling, sampling, feature_selection))
      new_task <- prepare_class_task(new_task,
                                     ...
                                     selected_attributes = features[[feature_selection_method]],
                                     add_weights = TRUE)
