...
class TestGenetwork:
    ...
    def test_genetwork_returns_zero_for_polska(self, runner: CliRunner, resources_dir: Path):
        result = runner.invoke(cli, [str(resources_dir / 'polska.xml')])
        assert not result.exception
        assert result.exit_code == 0

    def test_genetwork_returns_zero_for_germany50(self, runner: CliRunner, resources_dir: Path):
        result = runner.invoke(cli, [str(resources_dir / 'germany50.xml')])
        assert not result.exception
        assert result.exit_code == 0

    def test_genetwork_prints_something(self, runner: CliRunner):
